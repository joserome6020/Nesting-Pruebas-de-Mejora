# 📝 Registro de Refactorización (Fase 1 y Fase 2)
**Fecha:** Julio 2026
...
### Nueva Estructura:
- **`api_server.py`**: Ahora es solo un archivo "fachada" (entry point) de menos de 20 líneas. Importa los *routers* y arranca FastAPI.
...
4. **`api/routers/`**: Se utilizó un extractor AST (Abstract Syntax Tree) para mover de forma matemáticamente exacta las 39 rutas de la API a sus dominios específicos:
...
- **NO hubo pérdida de rutas:** Se validó de forma programática que las mismas 39 rutas siguen registradas exactamente bajo los mismos paths. 
...
- **`tab_nesting.py`**: Quedó reducido a ~600 líneas (50 métodos). Ahora actúa únicamente como "pegamento" de estos 5 Mixins y concentra el ciclo de vida principal del widget (constructor `__init__`, guardado de workspace y Helpers UI básicos).
