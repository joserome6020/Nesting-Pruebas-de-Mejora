...
# Fallback SOLO si no hay placas del calibre exacto.
...
class MotorNesting:
    ...
    def _espesor_con_tolerancia(val1, val2) -> bool:
        """Fallback relativo (15%). Solo usar si no hay match exacto de inventario."""
        if MotorNesting._espesor_exacto(val1, val2):
            return True
        n1 = MotorNesting._thickness_inches_for_match(val1)
        n2 = MotorNesting._thickness_inches_for_match(val2)
        if n1 is None or n2 is None:
            return False
        mayor = max(abs(n1), abs(n2))
        if mayor <= 1e-9:
            return True
        return abs(n1 - n2) <= (mayor * float(THICKNESS_TOLERANCE_PCT))
    ...
    def _clasificar_placas_por_calibre(
        ...
        """
        Prioridad:
          1) placas del mismo calibre (exacto)
          2) si no hay, tolerancia % (puede saltar de gauge)
        """
        placas_exactas_emp: list[dict] = []
        placas_exactas_prov: list[dict] = []
        placas_tol_emp: list[dict] = []
        placas_tol_prov: list[dict] = []
        ...
        for placa in datos_placas or []:
            ...
            elif self._espesor_con_tolerancia(req_cal, p_cal):
                (placas_tol_emp if es_emp else placas_tol_prov).append(
                    datos_placa_dict
                )
        ...
        toleradas = placas_tol_emp if placas_tol_emp else placas_tol_prov
        toleradas.sort(
            key=lambda x: (x.get("delta_thk", 999.0), x["precio_lb"], x["precio"])
        )
        return toleradas, "tolerancia"
    ...
    def _procesar_grupo_parallel(
        ...
        try:
            ...
            piezas = smart_seed_order(piezas or [])
    ...
    def _procesar_grupo_parallel_impl(
        ...
        if hojas_finales:
            ...
            # Tolerancia = salto de calibre → agrupar bajo el calibre de la placa.
            clave_out = clave
            if match_mode == "tolerancia":
                from collections import Counter

                cals = [
                    str(h.get("placa_cal") or "").strip()
                    for h in hojas_finales
                    if str(h.get("placa_cal") or "").strip()
                ]
                if cals:
                    cal_placa = Counter(cals).most_common(1)[0][0]
                    if not self._espesor_exacto(req_cal, cal_placa):
                        clave_out = f"{cal_placa}_{req_mat}" if req_mat else cal_placa
                        resultado_placas["calibre_placa"] = cal_placa
                        resultado_placas["reclasificado_por_placa"] = True
                        _dbg_nesting(
                            f"[RECLASIF-CAL] pieza={clave} -> placa={clave_out} | "
                            f"match=tolerancia"
                        )
