...
def _jobs_fuente_demanda_swo(cursor, swo_id: str) -> list[str]:
    """Jobs reales ligados a una SWO (reporte_cortes + diccionario_swo)."""
    ...
    def _add(j: str) -> None:
        ...
        if not jn:
            return
        key = _norm_job(jn)
        if key in vistos or key.startswith("SWO"):
            return
        vistos.add(key)
        jobs.append(jn)
    ...
    if cursor is not None:
        ...
        try:
            cursor.execute(
                """
                SELECT DISTINCT TRIM(job_numero) AS job
                FROM diccionario_swo
                WHERE TRIM(swo_id) = %s
                  AND job_numero IS NOT NULL
                  AND BTRIM(job_numero) <> ''
                """,
                (swo,),
            )
            for row in cursor.fetchall() or []:
...
def resolver_jobs_fuente_demanda(app, job: str, cursor=None) -> list[str]:
    ...
    SWO-xxx → jobs fusionados (ej. 251007).
    """
...
def validar_mrl_swo_canonica_tras_export(swo_id: str) -> tuple[bool, str]:
    """
    Barrera final antes de VSM/ContPAQ: la MRL SWO debe corresponder
    exactamente al plan persistido para toda la demanda fusionada.
    """
    swo = str(swo_id or "").strip()
    if not swo:
    ...
    try:
        ...
        if not _plan_largos_valido(plan):
            return False, f"SWO {swo}: no existe un plan canónico utilizable."
        ...
        if actuales != esperadas:
            return (
                False,
                f"SWO {swo}: MRL no coincide con plan canónico "
                f"(esperadas={sum(esperadas.values())}, actuales={sum(actuales.values())}).",
            )
        return (
            True,
