...
class ExportMixin:
    ...
    def _centralizar_exportacion_confirmada(
        ...
        *,
            ...
            with psycopg2.connect(**db_conf) as conn:
                with conn.cursor() as cur:
                    if es_swo:
                        ...
                        cur.execute(
                            """
                            SELECT DISTINCT TRIM(job_numero)
        ...
        jobs_involved = {
            ...
            if str(j or "").strip() and not str(j).strip().upper().startswith("SWO")
        }
        ...
        if _pendiente(stage):
            resultado_po = (
                ...
                if es_swo
                else trigger_pedido_po(job_activo)
            )
            if not resultado_po:
    ...
    def reanudar_centralizacion_pendiente(self):
        ...
        es_swo = job_activo.startswith("SWO")
