...
from .plasma_compensator import _arc_points_from_bulge
...
class VisorDXF:
    ...
    def __init__(self, master_frame):
        ...
        # --- SECCIÓN 2: VISUALIZADOR (Fijo arriba) ---

        self.frame_seccion_2 = ctk.CTkFrame(master_frame, fg_color="transparent")

        self.frame_seccion_2.pack(side="top", fill="both", expand=True, padx=0, pady=0)

        

        ctk.CTkFrame(master_frame, height=2, fg_color="#333333").pack(side="top", fill="x", padx=10, pady=5)



        # --- SECCIÓN 3: TABLA DE DATOS (Fija abajo) ---

        self.frame_seccion_3 = ctk.CTkFrame(master_frame, height=250, fg_color="transparent")

        self.frame_seccion_3.pack(side="bottom", fill="x", padx=10, pady=10)

        self.frame_seccion_3.pack_propagate(False)
        ...
        self.canvas = FigureCanvasTkAgg(self.figure, master=self.frame_seccion_2)
        ...
        self.widget.bind("<Escape>", self._cancelar_cota_interactiva)
    ...
    def construir_tabla_3_columnas(self):
        ...
        """
        for i in range(2):
            self.frame_seccion_3.columnconfigure(i, weight=1)
        ...
        for r, (texto, attr_name) in enumerate(filas):
            ctk.CTkLabel(
                self.frame_seccion_3,
                text=texto,
                font=font_label,
                text_color=col_label
            ).grid(row=r, column=0, sticky="w", padx=12, pady=(4 if r == 0 else 2, 2))

            lbl = ctk.CTkLabel(
                self.frame_seccion_3,
                text="-",
                font=font_value,
                text_color=col_value,
                anchor="e"
            )
            lbl.grid(row=r, column=1, sticky="e", padx=12, pady=(4 if r == 0 else 2, 2))
    ...
    def actualizar_datos(self, min_x, max_x, min_y, max_y, perimetro, valido, area=None, referencia=""):
        ...
        self.lbl_width.configure(text=f"{ancho_in:.2f}\"")
        self.lbl_height.configure(text=f"{alto_in:.2f}\"")
        self.lbl_area.configure(text=f"{area_in2:.2f} in²")
        self.lbl_perim.configure(text=f"{perim_in:.2f}\"")
        self.lbl_ref.configure(text=str(referencia or "-"))

    def actualizar_info_extra(self, area_in2=None, referencia=None):
        if area_in2 is not None:
            self.lbl_area.configure(text=f"{float(area_in2):.2f} in²")
    ...
    def mostrar_patron_prueba(self):
        ...
        self.lbl_width.configure(text='-')
        self.lbl_height.configure(text='-')
        self.lbl_area.configure(text='-')
        self.lbl_perim.configure(text='-')
    ...
    def on_mouse_move(self, event):
        ...
        if self._is_panning and self._lims is not None:
            dx = (event.x - self._pan_start[0]) * (self._lims[0][1] - self._lims[0][0]) / max(1, self.widget.winfo_width())
            dy = (event.y - self._pan_start[1]) * (self._lims[1][1] - self._lims[1][0]) / max(1, self.widget.winfo_height())
    ...
    def on_click(self, event):
        ...
        if event.button == 3:
            ...
            width_txt = self.lbl_width.cget("text")
            height_txt = self.lbl_height.cget("text")
            area_txt = self.lbl_area.cget("text")
            perim_txt = self.lbl_perim.cget("text")
            ref_txt = self.lbl_ref.cget("text")
            self._rotacion_vista_deg = (self._rotacion_vista_deg + 90) % 360
            if self._ruta_actual:
                self.renderizar_dxf(self._ruta_actual)
                self.lbl_width.configure(text=width_txt)
                self.lbl_height.configure(text=height_txt)
                self.lbl_area.configure(text=area_txt)
                self.lbl_perim.configure(text=perim_txt)
                self.lbl_ref.configure(text=ref_txt)
            return
        if event.button != 1:
            return
    ...
    def _set_pan_cursor(self, mode):
        ...
        cursor = "hand1" if mode == "panning" else ""
        try:
            self.widget.configure(cursor=cursor)
        except Exception:
            pass
...
def generar_thumbnail(ruta_dxf, size=(50, 50)):
    ...
    try:
        ...
        img = Image.open(buf); return ctk.CTkImage(img, img, size=size)
