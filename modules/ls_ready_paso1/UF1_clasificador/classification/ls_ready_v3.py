...
FOLLOW_X_E1_FORMULA = "E1_new = E1_previous - (X_new - X_previous)"
UF1_MARGIN_MM = 0.0
...
def apply_ls_ready_v3(classified: Dict[str, Any], runtime_cfg=None, logs=None) -> Dict[str, Any]:
    ...
    data["schema_version"] = "carlos_rpa.piece_plan.v3.uf1_a_240x96"
...
def refresh_ls_ready_v3_metadata(data: Dict[str, Any], logs=None) -> Dict[str, Any]:
    ...
    # Un refresh solo es válido sobre previews que ya pertenecen al mapa R3.
    meta["robot"] = "R3"
    ...
    out["robot_programming"] = robot_programming
...
def _coordinate_mapping_notes() -> Dict[str, Any]:
    return {
        ...
        "bed": "A",
        "uf1_axis_meaning": {
            "x_uf1": "sheet long side inverted, 0..6096 mm",
            "y_uf1": "sheet short side inverted, 0..2438 mm",
        },
        "physical_to_dxf": {"left": "min_x", "right": "max_x", "bottom": "max_y", "top": "min_y"},
        "transform_reference": {
            "x_uf1": "sheet_width_mm - x_dxf",
            "y_uf1": "sheet_height_mm - y_dxf",
            "margin_mm": 0.0,
            "note": "Normalización MAX_X/MAX_Y y entradas resueltas en DXF antes de convertir a UF-1.",
        },
        "e1_follow_axis": "X_UF1",
    }
...
def _operation_order_policy() -> Dict[str, Any]:
    return {
        "mark": {
            ...
            "dxf_sort": {"primary": "x_descending", "secondary": "y_descending"},
        },
        "cut": {
            "mode": "type_priority_then_position",
            "coordinate_reference": "DXF_NORMALIZED_AND_UF1_PREVIEW",
            "type_priority": ["holes", "cut_inner", "cut_outer"],
            "holes": {
                "mode": "individual_paths",
                "physical_order": "x_priority_then_y_descending_then_distance",
                "dxf_sort": {"primary": "x_ascending", "secondary": "y_descending", "tertiary": "distance"},
                "e1_fixed_policy": "each_hole_uses_own_entry",
            },
            "cut_inner": {
                "mode": "individual_paths",
                "entry_strategy": "min_x_max_y_diagonal_plus2_x_minus2_y",
                "dxf_start": "min_x_max_y",
                "cut_in": "diagonal_plus2_x_minus2_y_inside_waste",
                "e1_fixed_policy": "each_inner_uses_own_entry",
            },
            "cut_outer": {"mode": "always_last", "dxf_start": "max_y_max_x"},
        },
    }
...
def _ls_generation_policy() -> Dict[str, Any]:
    return {
        ...
        "classifier_scope": "exclusive_r3_cama_A_UF1_240X96",
        "first_generated_point": "P[7] after fixed square A P[1]..P[6]",
        "line_count_meaning": "total_POS_points",
        "e1": {
            "limits_mm": dict(E1_LIMITS_MM),
            "allow_negative": True,
            "follow_x_formula": FOLLOW_X_E1_FORMULA,
            "follow_x_reference": "previous_POS_point",
            "initial_follow_x_reference": {"point": "P[6] of fixed square A", "note": "El generador toma el estado de P[6] antes del primer path no fijo."},
            "fixed_e1_operations": ["mark_text", "holes", "cut_inner"],
            "follow_x_operations": ["mark_figure", "cut_outer"],
        },
        "path_header": {
            "insert_once_before_each_path": ["UTOOL_NUM=1", "UFRAME_NUM=1"],
            "do_not_duplicate_as_previous_path_footer": True,
            "tooling_note": "Cama A usa UF-1 y conserva UTOOL/UT=1.",
        },
        "geometry_contract": {
            "holes": {
        ...
        "final_home": {"required": True, "uses_new_final_POS_point": True, "joint_values": {"UF": 0, "UT": 1, "J1": 0.0, "J2": -0.0, "J3": 0.0, "J4": 0.0, "J5": -90.0, "J6": 0.0, "E1": 200.0}},
    }
...
def _load_motion_map(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    path = os.path.join(root, "config", "motion_maps", "R3_A_240X96_E1_J2_V1.json")
    if os.path.isfile(path):
        with open(path, "r", encoding="utf-8") as f:
            return json.load(f)
    return {
        "map_id": "R3_A_240X96_E1_J2_V1",
        "meta": {"robot": "R3", "cama": "A", "placa": "240x96"},
        "selection": {},
...
def _motion_preview(pt_dxf: Optional[Sequence[float]], robot_programming: Dict[str, Any], motion_map: Dict[str, Any]) -> Dict[str, Any]:
    if not pt_dxf:
        return {"map_id": motion_map.get("map_id"), "coordinate_reference": "DXF_NORMALIZED", "e1_preview": None, "j2_preview": None, "e1_limits_mm": dict(E1_LIMITS_MM), "e1_within_limits": None, "used_for_correction": False, "v4_transition_ready": True}
    x_dxf, y_dxf = float(pt_dxf[0]), float(pt_dxf[1])
    pt_uf1 = _to_uf1(pt_dxf, robot_programming)
    row, row_points = _select_motion_row(y_dxf, motion_map)
    node, extrapolated = _select_motion_node(row_points, x_dxf)
    if node is None:
        e1 = j2 = delta_x = node_id = None
    else:
    ...
    if row == "C" and j2_risk:
        rows = _motion_rows(motion_map)
        candidates = [(abs(y_dxf - _motion_row_y(rows, alt)), alt) for alt in ("B", "D") if alt in rows]
        if candidates:
            fallback = min(candidates)[1]
    return {
        "map_id": motion_map.get("map_id"), "coordinate_reference": "DXF_NORMALIZED",
        "sample_point_dxf": _round_point(pt_dxf), "sample_point_uf1_preview": _round_point(pt_uf1),
        "selected_row": row or None, "reference_node": node_id,
        ...
        "delta_x_mm": _round_num(delta_x), "e1_source": "selected_row_lower_x_node_plus_delta_x",
        "e1_formula": "E1_start = E1_node + (X_entry - X_node)", "e1_preview": _round_num(e1),
        "j2_source": "selected_reference_node", "j2_preview": _round_num(j2),
        "j2_warning_deg": J2_WARNING_DEG, "j2_physical_limit_deg": J2_PHYSICAL_LIMIT_DEG,
        "j2_risk_warning": bool(j2_risk), "j2_limit_exceeded": bool(j2_exceeded),
        "fallback_row_if_required": fallback, "extrapolated": bool(extrapolated),
        "extrapolation_allowed_inside_sheet": True, "e1_limits_mm": dict(E1_LIMITS_MM),
        "e1_within_limits": _e1_within_limits(e1),
        "requires_adjustment": bool(j2_exceeded or _e1_within_limits(e1) is False),
        "used_for_correction": False, "v4_transition_ready": True,
    }
...
def _resolve_optim_v3(
    ...
    limit_y = min(float(p[1]) for p in points)
...
def _motion_preview_optim(
    ...
    try:
        ...
        return {
            ...
            "sample_point_uf1_preview": _round_point(pt_uf1),
...
def _piece_outer_anchor(piece: Dict[str, Any]) -> Point:
    ...
    anchor = _choose_point(points, "dxf_max_x_max_y")
    if anchor:
        return anchor
    bb = _bbox(piece)
...
def _resolve_piece_groups_and_orders(pieces: List[Dict[str, Any]]) -> Tuple[Dict[str, Any], List[str], List[str]]:
    ...
    for idx, col in enumerate(columns, start=1):
        ...
        out_cols.append({
            "column_id": idx, "reference_anchor_x_dxf": _round_num(col["reference_x"]),
            "column_band_mm": COLUMN_BAND_MM, "piece_ids_bottom_to_top": ids,
            "anchors_dxf": [{"piece_id": str(p.get("id")), "anchor": _round_point(_piece_outer_anchor(p))} for p in col["pieces"]],
        })
        cut_order.extend(ids)
    mark_order: List[str] = []
    for col in reversed(out_cols):
        mark_order.extend(col["piece_ids_bottom_to_top"])
    return {"columns": out_cols, "grouping": "outer_entry_anchor_fixed_100mm", "row_order_dxf": "y_descending"}, cut_order, mark_order
...
def _attach_ls_generation(step: Dict[str, Any]) -> Dict[str, Any]:
    ...
    e1_mode = str(mm.get("e1_mode") or "follow_x")
    use_fixed = e1_mode in {"fixed", "fixed_from_group_anchor"}
    fixed_value = _e1_fixed_from_motion(mm) if use_fixed else None
    j2_preview = _j2_from_motion(mm)
    if use_fixed:
        e1_block = {"mode": "fixed", "use_e1_fixed": True, "e1_fixed_mm": _round_num(fixed_value), "source": "motion_map_anchor", "limits_mm": dict(E1_LIMITS_MM), "within_limits": _e1_within_limits(fixed_value), "allow_negative": True}
    else:
        e1_block = {"mode": "follow_x", "use_e1_fixed": False, "e1_fixed_mm": None, "source": "incremental_x_from_previous_POS_point", "formula": FOLLOW_X_E1_FORMULA, "limits_mm": dict(E1_LIMITS_MM), "allow_negative": True, "preview_at_entry_mm": mm.get("e1_preview_at_entry", mm.get("e1_preview"))}
    step["ls_generation"] = {
        "ready_for_generator": True, "operation": step.get("op"), "path_name": step.get("path_name"), "e1": e1_block,
        "audit": {"map_id": mm.get("map_id"), "selected_row": mm.get("selected_row"), "reference_node": mm.get("reference_node"), "delta_x_mm": mm.get("delta_x_mm"), "j2_preview_deg": _round_num(j2_preview), "j2_source": mm.get("j2_source") or "motion_anchor", "j2_risk_warning": mm.get("j2_risk_warning"), "j2_limit_exceeded": mm.get("j2_limit_exceeded"), "extrapolated": mm.get("extrapolated"), "requires_adjustment": mm.get("requires_adjustment"), "motion_anchor": mm.get("motion_anchor")},
...
def _enrich_mark_step(step: Dict[str, Any], robot_programming: Dict[str, Any], motion_map: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if step.get("op") == "mark_text":
        strategy, target = "mark_text_dxf_min_x_max_y", "dxf_min_x_max_y"
        ...
        policy_id, e1_mode = "R3_A_240X96_MARK_TEXT_ENTRY_V1", "fixed"
    else:
        # Cama A: inicio físico abajo → arriba = esquina DXF (X_max, Y_max) y
        # recorrido Y_max → Y_min. No altera coordenadas; solo el índice de inicio.
        strategy = "mark_figure_dxf_max_x_max_y"
        ordered, start = _order_figure_start_corner(
            strokes,
            corner_mode="dxf_max_x_max_y",
            prefer_next="dxf_decreasing_y",
        )
        continuous_points = []
        policy_id, e1_mode = "R3_A_240X96_MARK_FIGURE_ENTRY_V1", "follow_x"
    uf1 = _to_uf1(start, robot_programming) if start else None
    step["entry"] = {"strategy": strategy, "policy_id": policy_id, "coordinate_stage": "dxf_global_before_uf_transform", "requires_cut_in": False, "start_point_dxf": _round_point(start) if start else None, "start_point_uf1_preview": _round_point(uf1) if uf1 else None}
    geometry_type = "text_continuous" if step.get("op") == "mark_text" and step.get("laser_mode") == "continuous" else "strokes"
    step["geometry"] = {"type": geometry_type, "source_strokes_dxf": strokes, "ordered_strokes_dxf": ordered, "ordered_by": strategy, "was_reordered": True}
    if step.get("op") == "mark_text" and continuous_points:
        ...
        step["path_rules"] = {"must_close": False, "is_open_path": True, "allow_inserted_start_point": False, "laser_continuous_for_group": True}
    else:
    ...
    step["motion_mapping"] = {**preview, "e1_mode": e1_mode, "e1_scope": "text_block" if e1_mode == "fixed" else "path", "e1_value": preview.get("e1_preview") if e1_mode == "fixed" else None, "e1_preview_at_entry": preview.get("e1_preview"), "j2_preview_at_entry": preview.get("j2_preview"), "motion_anchor": {"source": "entry_start_point", "start_point_dxf": _round_point(start) if start else None, "start_point_uf1_preview": _round_point(uf1) if uf1 else None, "e1_preview": preview.get("e1_preview"), "j2_preview": preview.get("j2_preview")}}
    _attach_ls_generation(step)
    step["order_key"] = {"phase": "mark", "coordinate_reference": "DXF_NORMALIZED", "sort_mode": "right_to_left_then_bottom_to_top", "sort_x_dxf": _round_num(float(start[0])) if start else None, "sort_y_dxf": _round_num(float(start[1])) if start else None, "sort_x_uf1_preview": _round_num(uf1[0]) if uf1 else None, "sort_y_uf1_preview": _round_num(uf1[1]) if uf1 else None}
    return step
...
def _enrich_holes_step(step: Dict[str, Any], robot_programming: Dict[str, Any], motion_map: Dict[str, Any]) -> Dict[str, Any]:
    ...
    enriched.sort(key=lambda c: (float((c.get("order_key") or {}).get("primary_x_dxf") or 0.0), -float((c.get("order_key") or {}).get("secondary_y_dxf") or 0.0), float((c.get("order_key") or {}).get("tertiary_distance_mm") or 0.0)))
    for idx, contour in enumerate(enriched, start=1):
        contour["hole_id"], contour["order_index"] = "H{0:02d}".format(idx), idx
        mm = contour.get("motion_mapping") or {}
        contour["motion_mapping"] = {**mm, "e1_mode": "fixed", "e1_scope": "single_hole", "e1_value": mm.get("e1_preview"), "individual_e1_fixed": True}
    ...
    step["geometry"] = {"type": "closed_contours", "source_contours_dxf": contours, "ordered_contours_dxf": enriched, "ordered_by": "holes_x_ascending_y_descending_distance_240x96", "was_reordered": True}
    step["hole_order_policy"] = _operation_order_policy()["cut"]["holes"]
    first_mm = (first.get("motion_mapping") or {}) if first else {}
    step["motion_mapping"] = {**first_mm, "e1_mode": "fixed", "e1_scope": "single_hole", "e1_value": first_mm.get("e1_preview"), "individual_hole_path": True, "grouping_disabled": True, "motion_anchor": {"source": "own_hole_entry", "hole_id": first.get("hole_id") if first else None, "start_point_dxf": (first.get("entry") or {}).get("start_point_dxf") if first else None, "e1_preview": first_mm.get("e1_preview"), "j2_preview": first_mm.get("j2_preview")} if first else None}
    _attach_ls_generation(step)
    step["order_key"] = {"phase": "cut", "type_priority": 1, "sort_mode": "hole_x_asc_y_desc_distance", "primary_x_dxf": (first.get("order_key") or {}).get("primary_x_dxf") if first else None, "secondary_y_dxf": (first.get("order_key") or {}).get("secondary_y_dxf") if first else None}
...
def _apply_hole_arc_contract(c: Dict[str, Any], robot_programming: Dict[str, Any]) -> Optional[Point]:
    ...
    start_uf1 = _to_uf1(start, robot_programming) if start else None
    cut_in_uf1 = _to_uf1(cut_in, robot_programming) if cut_in else None
    ...
    c["entry"] = {
        ...
        "policy_id": "R3_A_240X96_HOLE_ENTRY_ARC_V2",
        ...
        "first_move_vector_uf1": [1.0, 0.0],
        "start_point_uf1_preview": _round_point(start_uf1) if start_uf1 else None,
        "cut_in_point_uf1_preview": _round_point(cut_in_uf1) if cut_in_uf1 else None,
        "cut_in_validation": {
    ...
    if eligible:
        ...
        c["generator_path_dxf"] = {
            ...
            "shape_type": "hole_fallback",
            "coordinate_stage": "dxf_global_before_uf_transform",
            "fit": fit_audit,
            "motion_profile": motion_profile,
            "entry": copy.deepcopy(c.get("entry")),
            "cut_points_dxf": fallback_ordered,
            "must_close": True,
            "close_to_start": True,
...
def _enrich_single_hole(contour: Dict[str, Any], idx: int, robot_programming: Dict[str, Any], motion_map: Dict[str, Any]) -> Dict[str, Any]:
    ...
    posture_sample = [start[0], max(p[1] for p in points)] if (start and points) else start
    c["motion_mapping"] = _motion_preview(posture_sample, robot_programming, motion_map)
    c["order_key"] = {
        "coordinate_reference": "DXF_NORMALIZED",
        "sort_mode": "x_ascending_y_descending_distance",
...
def _enrich_cut_inner_step(step: Dict[str, Any], robot_programming: Dict[str, Any], motion_map: Dict[str, Any]) -> Dict[str, Any]:
    step = copy.deepcopy(step)
    contours = list((step.get("geometry") or {}).get("contours") or [])
    enriched = [_enrich_inner_contour(c, idx, robot_programming, motion_map) for idx, c in enumerate(contours, start=1)]
    ...
    step["geometry"] = {"type": "closed_contours", "source_contours_dxf": contours, "ordered_contours_dxf": enriched, "ordered_by": "inner_x_ascending_y_descending", "was_reordered": True}
    first_mm = (first.get("motion_mapping") or {}) if first else {}
    ...
    _attach_ls_generation(step)
    start = (step.get("entry") or {}).get("start_point_dxf", [None, None])
    step["order_key"] = {"phase": "cut", "type_priority": 2, "sort_mode": "cut_inner_x_asc_y_desc", "primary_x_dxf": start[0], "secondary_y_dxf": start[1]}
    step["path_rules"] = {"must_close": True, "close_to_start": True, "is_open_path": False, "allow_inserted_start_point": True}
    return step
...
def _apply_slot_arc_contract(
    ...
    start = min(candidates, key=lambda p: (-round(float(p[1]), 2), -round(float(p[0]), 2))) if candidates else None
    if start is None:
        return None
    ...
    points = _contour_points(c)
    fallback_ordered = _order_closed_points_from_start(points, start, prefer_next="dxf_decreasing_y")
    cut_in = _diagonal_cut_in_point_dxf(start, dx_mm=2.0, dy_mm=-2.0)
    start_uf1 = _to_uf1(start, robot_programming)
    cut_in_uf1 = _to_uf1(cut_in, robot_programming)
    cut_in_inside = _point_in_polygon(cut_in, points)

    c["entry"] = {
        "strategy": "inner_slot_min_x_max_y_diagonal_plus2_x_minus2_y",
        "policy_id": "R3_A_240X96_SLOT_ENTRY_ARC_V1",
        "coordinate_stage": "dxf_global_before_uf_transform",
        "requires_cut_in": True,
        ...
        "first_move_vector_uf1": [0.7071, -0.7071],
        "start_point_uf1_preview": _round_point(start_uf1),
        "cut_in_point_uf1_preview": _round_point(cut_in_uf1),
        "cut_in_validation": {
...
def _enrich_inner_contour(contour: Dict[str, Any], idx: int, robot_programming: Dict[str, Any], motion_map: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if start is None:
        start = _choose_point(points, "dxf_min_x_max_y")
        ...
        cut_in_uf1 = _to_uf1(cut_in, robot_programming) if cut_in else None
        cut_in_inside = _point_in_polygon(cut_in, points) if cut_in else None
        c["entry"] = {"strategy": "inner_min_x_max_y_diagonal_plus2_x_minus2_y", "policy_id": "R3_A_240X96_INNER_CUT_ENTRY_V1", "coordinate_stage": "dxf_global_before_uf_transform", "requires_cut_in": True, "cut_in_mm": _round_num(math.hypot(2.0, 2.0)), "cut_in_type": "diagonal", "cut_in_dx_mm": 2.0, "cut_in_dy_mm": -2.0, "cut_in_angle_deg": -45.0, "start_point_dxf": _round_point(start) if start else None, "cut_in_point_dxf": _round_point(cut_in) if cut_in else None, "first_move_vector_dxf": [-0.7071, 0.7071], "first_move_vector_uf1": [0.7071, -0.7071], "start_point_uf1_preview": _round_point(start_uf1) if start_uf1 else None, "cut_in_point_uf1_preview": _round_point(cut_in_uf1) if cut_in_uf1 else None, "cut_in_validation": {"inside_waste_polygon": cut_in_inside, "validation_method": "point_in_polygon_dxf", "warning_if_false": "Review inner cut entry before LS generation."}}
...
def _enrich_cut_outer_step(step: Dict[str, Any], robot_programming: Dict[str, Any], motion_map: Dict[str, Any]) -> Dict[str, Any]:
    ...
    points = _contour_points(contour)
    start = _choose_point(points, "dxf_max_y_max_x")
    ordered = _order_closed_points_from_start(points, start, prefer_next="dxf_decreasing_x")
    cut_in = [start[0] + 3.0, start[1]] if start else None
    start_uf1 = _to_uf1(start, robot_programming) if start else None
    cut_in_uf1 = _to_uf1(cut_in, robot_programming) if cut_in else None
    entry = {"strategy": "outer_dxf_max_y_max_x", "policy_id": "R3_A_240X96_OUTER_CUT_ENTRY_V3", "coordinate_stage": "dxf_global_before_uf_transform", "requires_cut_in": True, "cut_in_mm": 3.0, "start_point_dxf": _round_point(start) if start else None, "cut_in_point_dxf": _round_point(cut_in) if cut_in else None, "first_move_vector_dxf": [-1.0, 0.0], "first_move_vector_uf1": [-1.0, 0.0], "start_point_uf1_preview": _round_point(start_uf1) if start_uf1 else None, "cut_in_point_uf1_preview": _round_point(cut_in_uf1) if cut_in_uf1 else None}
    contour["ordered_points_dxf"] = ordered
    step["entry"] = entry
    step["geometry"] = {"type": "closed_contour", "source_points_dxf": points, "ordered_points_dxf": ordered, "start_point_dxf": _round_point(start) if start else None, "contour": contour, "closed": True, "was_reordered": True, "ordered_by": "outer_dxf_max_y_max_x_then_decreasing_x"}
    preview = _motion_preview_optim(start, points, "cut_outer", robot_programming, motion_map)
    step["motion_mapping"] = {**preview, "e1_mode": "follow_x", "e1_preview_at_entry": preview.get("e1_preview"), "j2_preview_at_entry": preview.get("j2_preview")}
    _attach_ls_generation(step)
...
def _mark_sort_key(step: Dict[str, Any]) -> Tuple[float, float, str]:
    ...
    x, y = ok.get("sort_x_dxf"), ok.get("sort_y_dxf")
...
def _order_figure_start_corner(
    strokes: List[Dict[str, Any]],
    corner_mode: str = "dxf_max_x_max_y",
    prefer_next: str = "dxf_decreasing_y",
) -> Tuple[List[Dict[str, Any]], Optional[Point]]:
    """Inicia la figura en el punto más cercano a la esquina objetivo del bbox DXF.

    Cama A: (X_max, Y_max) = arranque físico abajo, recorrido Y_max→Y_min.
    No altera coordenadas: solo cambia el índice de inicio y el sentido.
    """
    if not strokes:
        return [], None
    ordered, _ = _order_strokes_by_target(strokes, corner_mode)
    ...
    max_x = max(float(p[0]) for p in body)
    min_x = min(float(p[0]) for p in body)
    max_y = max(float(p[1]) for p in body)
    min_y = min(float(p[1]) for p in body)
    if corner_mode == "dxf_max_x_max_y":
        start = min(body, key=lambda p: (-round(float(p[1]), 2), -round(float(p[0]), 2)))
    elif corner_mode == "dxf_max_x_min_y":
        start = min(body, key=lambda p: (round(float(p[1]), 2), -round(float(p[0]), 2)))
    elif corner_mode == "dxf_min_x_max_y":
        start = min(body, key=lambda p: (-round(float(p[1]), 2), round(float(p[0]), 2)))
    else:
        start = min(body, key=lambda p: (round(float(p[1]), 2), round(float(p[0]), 2)))
...
def _choose_point(points: Sequence[Sequence[float]], mode: str) -> Optional[Point]:
    ...
    if mode == "dxf_max_x_max_y":
        return max(pts, key=lambda p: (float(p[0]), float(p[1])))
    if mode == "dxf_max_y_max_x":
        return max(pts, key=lambda p: (float(p[1]), float(p[0])))
    return pts[0]
...
def _to_uf1(pt: Sequence[float], robot_programming: Dict[str, Any]) -> Point:
    """DXF global normalizado -> UF-1 cama A.

    X_UF1 = sheet_width - X_DXF
    Y_UF1 = sheet_height - Y_DXF
    """
    width = float(robot_programming.get("sheet_width_mm") or 0.0)
    height = float(robot_programming.get("sheet_height_mm") or 0.0)
    margin = float(robot_programming.get("margin_mm") if robot_programming.get("margin_mm") is not None else UF1_MARGIN_MM)
    return [_round_num(width - float(pt[0]) - margin), _round_num(height - float(pt[1]) - margin)]
