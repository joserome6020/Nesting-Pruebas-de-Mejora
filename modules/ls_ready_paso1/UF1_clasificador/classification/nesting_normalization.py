# -*- coding: utf-8 -*-
"""Normalización del nesting 240x96 cama A antes de entradas y mapa E1/J2."""
...
def normalize_nesting_geometry(cut_outer, cut_inner, mark, sheet_info, margin_mm=20.0):
    """Mueve toda la geometría útil hacia X máximo y Y máximo.

    En cama A/UF-1 ambos ejes se invierten al convertir:
      X_UF1 = sheet_width - X_DXF
      Y_UF1 = sheet_height - Y_DXF

    Llevar el nesting a MAX_X/MAX_Y lo deja cerca del origen UF-1. El margen
    de normalización es 20 mm y no debe confundirse con un margen LS adicional.
    """
    sheet = sheet_info or {}
    ...
    dy = height - effective_margin_y - max_y
    new_min_x = min_x + dx
    new_min_y = min_y + dy
    if new_min_x < -1e-6 or new_min_y < -1e-6:
        raise ValueError(
            "Tras anclar a max-X/max-Y con margen adaptativo ({:.3f}, {:.3f}) mm, el nesting sale de la placa "
            "(min_x={:.3f}, min_y={:.3f}; placa {:.3f}x{:.3f}).".format(
                effective_margin_x, effective_margin_y, new_min_x, new_min_y, width, height
            )
        )
