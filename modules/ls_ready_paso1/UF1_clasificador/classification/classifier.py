...
Flujo:
  ...
  2. Crear piezas A..Z para cama A (mark der->izq en DXF)
  3. Asignar mark / cut_inner a cada pieza
  4. build_piece_plan() por pieza -> local_plan
  5. stitch_global_plan() -> plan.mark (X decreciente) / plan.cut (X creciente), exclusivo cama A/UF-1
"""
import copy
import json
import os
...
def _normalize_meta_for_ls(meta):
    ...
    if original_bed and str(original_bed).upper() != "A":
...
def _uframe_from_bed(_bed=None):
    """Clasificador exclusivo cama A: siempre UF-1."""
    return 1
...
def _build_robot_programming(meta, runtime_cfg):
    ...
    margin = float(getattr(runtime_cfg, "LS_MARGIN_MM", 0.0))
    return {
        "robot": 3,
        "bed": "A",
        "uframe": 1,
        ...
        "coord_stage": "before_uf1_transform",
        "sheet_width_mm": round(width, 4),
        "sheet_height_mm": round(height, 4),
        "margin_mm": margin,
        "sheet_width_transform_mm": round(width - margin, 4),
        "sheet_height_transform_mm": round(height - margin, 4),
        "transform": {
            "x_uf1": "sheet_width_mm - x_dxf - margin_mm",
            "y_uf1": "sheet_height_mm - y_dxf - margin_mm",
        },
        "notes": [
            "Clasificador exclusivo para robot R3 / cama A / UF-1.",
            "La geometría se normaliza hacia MAX_X/MAX_Y con offset de 20 mm.",
            "El margen LS adicional es 0 mm; las entradas ya se resuelven en DXF.",
            "Para 240x96 el alto normalizado de programación es 2438.0 mm.",
        ],
    }
...
def classify_json_data(raw_data, source_path="", logs=None):
    ...
    if bool(getattr(runtime_cfg, "NORMALIZE_NESTING", False)):
        cut_outer_raw, raw_inner, raw_mark, normalization = normalize_nesting_geometry(
            ...
            raw_mark,
            sheet_info,
            margin_mm=float(getattr(runtime_cfg, "NORMALIZATION_MARGIN_MM", 20.0)),
        )
        logs.append(
