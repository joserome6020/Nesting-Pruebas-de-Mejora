...
def build_piece_mark_plan(piece_id, mark_strokes, cfg, logs=None):
    ...
    disable_text = bool(getattr(cfg, "MARK_DISABLE_TEXT", False))

    if disable_text:
        # Sin detección de texto: todo el grabado se trata como figuras.
        text_strokes = []
        figure_candidates = list(mark_strokes or [])
        if logs is not None:
            logs.append(
                "piece {0} mark_text_detection=disabled figures={1}".format(
                    piece_id, len(figure_candidates)
                )
            )
    else:
    ...
    # Guardia anti-falso-texto: el texto real debe formar un bloque compacto.
    # Si los trazos inicialmente detectados como texto abarcan una dimensión
    # mayor al límite configurado, pertenecen a marcas/figuras dispersas y se
    # regresan a figure_candidates. El texto compacto conserva E1 fijo.
    if text_strokes:
        max_text_long_dim = float(getattr(cfg, "TEXT_COMPONENT_MAX_LONG_DIM_MM", 700.0))
        xs0 = []
        ys0 = []
        for stroke in text_strokes:
            bbox = stroke.get("bbox_dxf") or []
            if len(bbox) >= 4:
                xs0.extend([float(bbox[0]), float(bbox[2])])
                ys0.extend([float(bbox[1]), float(bbox[3])])
    ...
    if figure_strokes:
        if use_components:
            ...
            if bool(getattr(cfg, "FIGURE_ONE_STEP_PER_COMPONENT", True)):
                ...
                # v3.3: un solo paso mark_figure por pieza, con un stroke por
                # componente/figura reconstruida. El generador v3.2 lo escribe como
                # figure_group: una llegada/salida y un ON/OFF por componente.
