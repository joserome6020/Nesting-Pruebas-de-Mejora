...
Flujo unificado para pruebas:
    ...
    DXF -> JSON crudo -> LS READY V3 R3 240X96 UF-1 A
    JSON crudo -> LS READY V3 R3 240X96 UF-1 A
...
def _upgrade_classified_json_to_ls_ready(json_path: str, output_path: str, logs: list) -> dict:
    ...
    schema_v3 = str(data.get("schema_version") or "") == "carlos_rpa.piece_plan.v3.uf1_a_240x96"
    ...
    already_r3 = map_id == "R3_A_240X96_E1_J2_V1" and map_robot == "R3" and programming_robot == "3"

    # Solo refrescar sin reordenar cuando los previews E1/J2 ya fueron calculados con R3.
    if schema_v3 and data.get("validation") and already_r3:
        ...
        logs.append("ls_ready_v3_r3_uf1_a: input_already_r3 refreshed_without_reordering")
        return data

    # Cualquier classified anterior, incluido R4, se reconstruye desde legacy/raw
    ...
    if any(legacy.get(k) for k in ("cut_outer", "cut_inner", "mark")):
        ...
        logs.append("ls_ready_v3_r3_uf1_a: rebuilt_from_legacy_raw")
        return rebuilt
    ...
    # Clasificador exclusivo cama A: siempre reconstruir robot_programming para UF-1,
...
def main(argv=None) -> int:
    ...
    try:
        ...
        if ext == ".json" and _json_kind(json_path) == "classified":
            print("JSON ya clasificado detectado. Actualizando/confirmando LS READY V3 R3 240X96 UF-1 A:", final_output)
            ...
            print("Clasificando a LS READY V3 R3 240X96 UF-1 A:", final_output)
    ...
    print("=" * 60)
    print("LS READY V3 R3 240X96 UF-1 A OK")
