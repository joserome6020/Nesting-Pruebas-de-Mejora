# -*- coding: utf-8 -*-
"""Resolución de postura V4 para R4 / cama B / UF-2 / placa 240x96.
...
En UF-2 (Cama B / R4), el avance de corte y grabado es en dirección Y_DXF creciente.
"""
...
class OptimPathResolverV4:
    def __init__(self, config_dir: str):
        ...
        map0_path = os.path.join(self.config_dir, "Mediciones 240x96 B, mapa V4 COLUMNA 0.txt")
        map1_path = os.path.join(self.config_dir, "Mediciones 240x96 B, mapa V4 COLUMNA 1.txt")
        ...
        *,
        ...
        if limit_y < entry_y - 0.05:
            return self._invalid_result(
                ...
                entry_y,
                limit_y,
                "path_motion_direction_must_be_dxf_increasing",
                path_id,
            )
        ...
        e1_mode = "fixed" if profile == "min_y_min_x" else "follow_y"
        result = {
            "profile": profile,
            "path_id": path_id,
            "status": status,
            ...
            "y_motion_direction_dxf": "increasing",
            "y_advance_mm": _round(limit_y - entry_y),
            ...
            "evaluated_samples": sorted(
                evaluated, key=lambda item: float(item["y_dxf"])
            ),
        }
        return result
