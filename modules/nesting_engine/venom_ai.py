...
def venom_enabled() -> bool:
    """Master switch. Default OFF (opt-in: ARGA_NEST_VENOM=1)."""
    import os

    v = (os.environ.get("ARGA_NEST_VENOM") or "0").strip().lower()
...
def apply_smart_polisher(
    ...
    *,
    ...
    # --- Cierre de bandas (filas/columnas): densifica aire entre clusters ---
    band_stats = {"bands_y": 0, "bands_x": 0, "mm_y": 0.0, "mm_x": 0.0}
    try:
        from .venom_band_close import close_inter_band_gaps

        band_stats = close_inter_band_gaps(hoja, engine_id) or band_stats
        if int(band_stats.get("bands_y") or 0) + int(band_stats.get("bands_x") or 0) > 0:
            # Refill cavidades tras compactar filas (nuevos encajes posibles).
            try:
                from .venom_hole_fill import fill_host_cavities

                fill2 = fill_host_cavities(hoja, engine_id) or {}
                fill_stats["filled"] = int(fill_stats.get("filled") or 0) + int(
                    fill2.get("filled") or 0
                )
                fill_stats["area_filled"] = float(fill_stats.get("area_filled") or 0.0) + float(
                    fill2.get("area_filled") or 0.0
                )
            except Exception:
                pass
            # Refrescar items/polys para reward y pockets
            piezas = hoja.get("piezas", [])
            items = []
            for idx, p in enumerate(piezas):
                poly = p.get("poly_exact") or p.get("poly")
            ...
            mm_ahorrados_y += float(band_stats.get("mm_y") or 0.0)
            mm_ahorrados_x += float(band_stats.get("mm_x") or 0.0)
    except Exception:
        band_stats = {"bands_y": 0, "bands_x": 0, "mm_y": 0.0, "mm_x": 0.0}
    ...
    corridor_n = int(hoja.get("venom_corridor_fill") or 0)
    if corridor_n > 0:
        reward += 3.0 * corridor_n
    band_n = int(band_stats.get("bands_y") or 0) + int(band_stats.get("bands_x") or 0)
    if band_n > 0:
        reward += 4.0 * band_n + 0.002 * (
            float(band_stats.get("mm_y") or 0.0) + float(band_stats.get("mm_x") or 0.0)
        )
    ...
    log_msg = (
        ...
        f"Band: y={int(band_stats.get('bands_y') or 0)}/{float(band_stats.get('mm_y') or 0):.0f}mm "
        f"x={int(band_stats.get('bands_x') or 0)}/{float(band_stats.get('mm_x') or 0):.0f}mm | "
