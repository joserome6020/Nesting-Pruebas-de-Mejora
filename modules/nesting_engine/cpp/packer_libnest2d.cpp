...
/** Objetivo NfpPlacer estilo libnest2d: BOTTOM_LEFT + distancia al origen. */
double nfp_placer_score(double px, double py) {
...
bool colocar_pieza_nfp(
    ...
    for (const auto& var : variaciones) {
        ...
        for (const auto& anclaje : anclajes) {
            ...
            const double score = nfp_placer_score(px, py);
...
using SelectorFn = std::function<bool(const PieceIn&, const PieceIn&)>;

SelectorFn selector_fn(SelectorKind kind) {
    switch (kind) {
    case SelectorKind::LargestPerimeter:
        return [](const PieceIn& a, const PieceIn& b) {
            return bbox_perimeter(a.rings) > bbox_perimeter(b.rings);
        };
    case SelectorKind::LargestWidth: {
        return [](const PieceIn& a, const PieceIn& b) {
            const Bounds ba = bounds_of_rings(a.rings);
            const Bounds bb = bounds_of_rings(b.rings);
            return (ba.maxx - ba.minx) > (bb.maxx - bb.minx);
        };
    }
    case SelectorKind::LargestHeight: {
        return [](const PieceIn& a, const PieceIn& b) {
            const Bounds ba = bounds_of_rings(a.rings);
            const Bounds bb = bounds_of_rings(b.rings);
            return (ba.maxy - ba.miny) > (bb.maxy - bb.miny);
        };
    }
    case SelectorKind::SmallestArea:
        return [](const PieceIn& a, const PieceIn& b) {
            return piece_area(a) < piece_area(b);
        };
    case SelectorKind::LongestName:
        return [](const PieceIn& a, const PieceIn& b) {
            return a.nombre > b.nombre;
        };
    case SelectorKind::LargestArea:
    default:
        return [](const PieceIn& a, const PieceIn& b) {
            return piece_area(a) > piece_area(b);
        };
...
/** FirstFit: en cada paso ordena restantes con el selector y coloca la primera que cabe. */
std::pair<PlacementState, std::vector<PieceIn>> firstfit_nfp_pack(
    ...
    const SelectorFn cmp = selector_fn(selector);

    while (!pendientes.empty()) {
        std::stable_sort(pendientes.begin(), pendientes.end(), cmp);

        bool placed_any = false;
        for (auto it = pendientes.begin(); it != pendientes.end(); ++it) {
            if (colocar_pieza_nfp(*it, state, w_placa, h_placa, kerf_radio, margin_px, limit)) {
                pendientes.erase(it);
                placed_any = true;
                break;
            }
...
SelectorKind selector_for_iteration(int i, int total) {
    static const SelectorKind cycle[] = {
        SelectorKind::LargestArea,
        SelectorKind::LargestPerimeter,
        SelectorKind::LargestWidth,
        SelectorKind::LargestHeight,
        SelectorKind::SmallestArea,
        SelectorKind::LongestName,
    };
    const size_t n = sizeof(cycle) / sizeof(cycle[0]);
    if (i < static_cast<int>(n)) {
        return cycle[static_cast<size_t>(i)];
    }
    return cycle[static_cast<size_t>(i % static_cast<int>(n))];
}

}  // namespace

PackResult empaquetar_una_hoja_libnest2d(
    ...
    const int iteraciones = std::max(1, std::min(selector_iterations, 50));

    for (int i = 0; i < iteraciones; ++i) {
        const SelectorKind sel = selector_for_iteration(i, iteraciones);
        auto [state, restos] = firstfit_nfp_pack(
            piezas,
            w_placa,
            h_placa,
            kerf_override,
