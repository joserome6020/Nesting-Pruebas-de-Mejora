...
std::vector<std::uint8_t> cpu_collide_batch(
    ...
    if (!valid_mask(board, board_w, board_h) || !valid_mask(piece, piece_w, piece_h)) {
        std::fill(collided.begin(), collided.end(), 1);
        return collided;
    }
...
bool requested() {
    const char* value = std::getenv("ARGA_LAB_PILOT_CUDA");
    if (!value) {
        return false;
    }
...
std::string availability_detail() {
    ...
    return code == 0
        ? "CUDA runtime disponible para lab_pilot."
        : "CUDA runtime no disponible; code=" + std::to_string(code);
#else
    return "CUDA no incluido en este build del piloto; fallback CPU.";
#endif
}
